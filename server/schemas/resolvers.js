...
const resolvers = {
  ...
  Mutation: {
    ...
    newEmployee: async (
      parent,
      { empName, email, phone, password, isManager },
      context
    ) => {
    ...
    newSite: async (
      parent,
      { siteName, siteLocation, company, siteContact, sitePhone },
      context
    ) => {
      if (context.employee.isManager) {
        let addSite = await Site.create({
          siteName,
          siteLocation,
          company,
          siteContact,
    ...
    addRoster: async (
      parent,
      { dayDate, siteName, employees, comments },
      context
    ) => {
      if (context.employee.isManager) {
        ...
        let newroster = await Roster.find({ _id: roster._id })
          .populate("siteName")
          .populate("employees")
          .exec();
        roster = newroster[0];
